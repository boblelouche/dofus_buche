...
class OngletsMap:
    ...
    def print_map(self, maps):
        ...
        self.carreau = {}
        self.Terrain = tkinter.Canvas(
            self.onglet_map,
            height=self.size_carreau * (maps.height) * 2,
            width=self.size_carreau * (maps.width) * 2,
        )
        self.binairemap = []
        ...
        for cell in maps.cells:
            ...
            if i == 0 and j % 2 == 0:
                if j != 0 and j % 2 == 0:
                    liste2.append(1)
                self.Terrain.create_rectangle(
                    i * self.size_carreau * 2,
                    j * self.size_carreau,
                    (i + 1) * self.size_carreau * 2,
                    (j + 1) * self.size_carreau,
                    fill="black",
                )
            ...
            self.carreau[cell.CellID] = {
                "carreau": self.Terrain.create_rectangle(
                    i * self.size_carreau * 2,
                    j * self.size_carreau,
                    (i + 1) * self.size_carreau * 2,
                    (j + 1) * self.size_carreau,
    ...
    def clic(self, event):
        ...
        if self.carreau[cell_ID]:
            ...
            if self.carreau[cell_ID]["cell"].isInteractive:
                ...
                not self.character.deplacement.ismouving
                and not self.character.isharvest
            ):
                self.character.deplacement.deplacement(
                    self.character.cell.CellID,
                    cell_ID,
                    self.mapswidth,
                    self.carreau,
                    self.binairemap,
