...
class MainInterface():
    ...
    def create_notebook(self):
        self.onglets = ttk.Notebook(self.main)
        self.onglets.pack()
        ...
        self.tabsMap = OngletsMap(self.onglets)
        self.spellsTab = OngletsSorts(self.onglets)

    def base_start(self, character):
        self.vita = tk.Label(
            self.main, bg="red", text=character.vie_actuelle + " / " + character.vie_max
        )
        self.vita.pack()
        self.vita.place(relx=0.20, rely=0.90, relwidth=0.08, relheight=0.08)

        self.energie = tk.Label(
            self.main,
            bg="yellow",
        ...
        self.xp = tk.Label(
            self.main,
            bg="deep sky blue",
            text=character.xp_actuelle + " / " + character.xp_fin,
        )
        self.xp.pack()
        self.xp.place(relx=0.60, rely=0.90, relwidth=0.1, relheight=0.08)

        self.kamas = tk.Label(self.main, bg="orange", text=character.kamas)
        self.kamas.pack()
