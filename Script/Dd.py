...
class Monture:
    ...
    def vider_ressource_on_dd(self):
        ...
        if self.pers.window is not None:
            ...
            second_ressource_empty = get_pixel_color_on_pos(positions['second_ressource_perso_dd'])  == colors["inventory_no_ressource"]
            third_ressource_empty = get_pixel_color_on_pos(positions['third_ressource_perso_dd'])  == colors["inventory_no_ressource"]
            keyboard.press('ctrl')
            time.sleep(2)
    ...
    def prendre_ressource_on_dd(self):
        ...
        if self.pers.window is not None:
            ...
            second_ressource_empty = get_pixel_color_on_pos(positions['second_ressource_dd'])  == colors["inventory_no_ressource"]
            third_ressource_empty = get_pixel_color_on_pos(positions['third_ressource_dd'])  == colors["inventory_no_ressource"]
            # print(first_ressource_empty, second_ressource_empty )
    ...
    def check_if_dd_is_full(self):
        ...
        if self.pers.window is not None:
            ...
            if screenshot.getpixel(positions['inventory_dd_max']) == colors["inventory_full"]:
                self.dd_full = 1
                return "inventaire plein"
            else:
                self.dd_full = 0
            try:
                image_positions = list(pyautogui.locateAllOnScreen(files["full_dd_picture"], confidence=0.85))       
